    ...
    : IAuthService
{
    ...
    public async Task<BaseResponseDto<bool>> SignInAsync(
        SignInUserRequestDto request, CancellationToken token = default)
    {
        var email = request.Email.ToLowerInvariant();

        var emailValidatorResponse = EmailValidator.ValidateFormat(email);
        if (emailValidatorResponse.IsFailure)
        {
            ...
            return BaseResponseDto<bool>.Failure(400, $"Invalid email {email} format.");
        }

        if (!string.IsNullOrWhiteSpace(request.RequestedBy))
        {
            logger.LogInformation("{class}.{method} - Authenticated email {email} can not request user signin.",
                nameof(AuthService), nameof(SignInAsync), request.RequestedBy);

            return BaseResponseDto<bool>.Failure(
                401, $"Authenticated email {request.RequestedBy} can not request user signin.");
        }
        ...
        if (passwordValidatorResponse.IsFailure)
        {
            ...
            return BaseResponseDto<bool>.Failure(400, $"Invalid password format for email {email} .");
        }
        ...
        if (userDb is null)
        {
            ...
            return BaseResponseDto<bool>.Failure(404, $"Email {email} not found.");
        }
    ...
    public async Task<BaseResponseDto<bool>> SignOutAsync(
        RefreshTokenRequestDto request, CancellationToken token = default)
    {
        if (string.IsNullOrWhiteSpace(request.RequestedBy))
        {
            logger.LogInformation("{class}.{method} - Unauthenticated email {email} can not request user sign out.",
                nameof(AuthService), nameof(SignOutAsync), request.RequestedBy);

            return BaseResponseDto<bool>.Failure(
                401, $"Unauthenticated email {request.RequestedBy} can not request user sign out.");
        }
    ...
    public async Task<BaseResponseDto<bool>> SignUpAsync(
        SignUpUserRequestDto request, CancellationToken token = default)
    {
        var email = request.Email.ToLowerInvariant();
        ...
        if (!string.IsNullOrWhiteSpace(request.RequestedBy))
        {
            logger.LogInformation("{class}.{method} - Authenticated email {email} can not request user signup.",
                nameof(AuthService), nameof(SignUpAsync), request.RequestedBy);

            return BaseResponseDto<bool>.Failure(
                401, $"Authenticated email {request.RequestedBy} can not request user signup.");
        }
        ...
        if (await usersRepository.ExistsByAsync(x => x.Email == email, token))
        {
            ...
            return BaseResponseDto<bool>.Failure(409, $"Email {email} already used.");
        }
