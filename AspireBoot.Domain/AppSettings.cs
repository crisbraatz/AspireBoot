...
public static class AppSettings
{
    ...
    public static string AngularOrigin => s_configuration?["AspireBoot:AngularOrigin"] ?? "https://app.localhost:1443";
    ...
    private static string AppType => new(s_configuration?["AspireBoot:AppType"] ?? "Api");
    ...
    public static string CookieDomain => s_configuration?["AspireBoot:Cookie:Domain"] ?? "app.localhost";
    ...
    public static int CookieExpiresAfter
    {
        get
        {
            bool parseOk = int.TryParse(
                s_configuration?["AspireBoot:Cookie:ExpiresAfter"] ?? "7", AppLanguage.NumberFormat, out int days);
    ...
    public static int JwtExpiresAfter
    {
        ...
        {
            bool parseOk = int.TryParse(
                s_configuration?["AspireBoot:Jwt:ExpiresAfter"] ?? "60", AppLanguage.NumberFormat, out int minutes);
    ...
    public static bool ShouldMigrate => AppType is "Api";

    public static string? GetCookieDomainFor(string requestHost)
    {
        string configuredDomain = NormalizeCookieDomain(CookieDomain);
        if (string.IsNullOrWhiteSpace(configuredDomain))
            return null;

        return requestHost.Equals(configuredDomain, StringComparison.OrdinalIgnoreCase) ||
               requestHost.EndsWith($".{configuredDomain}", StringComparison.OrdinalIgnoreCase)
            ? configuredDomain
            : null;
    }
    ...
    private static string NormalizeCookieDomain(string? cookieDomain)
    {
        ...
        if (Uri.TryCreate(cookieDomain, UriKind.Absolute, out Uri? uri))
            return uri.Host;

        string normalized = cookieDomain.Trim().Trim('/');
        int schemeSeparator = normalized.IndexOf("://", StringComparison.OrdinalIgnoreCase);
        if (schemeSeparator >= 0)
            normalized = normalized[(schemeSeparator + 3)..];

        int slashIndex = normalized.AsSpan().IndexOf('/');
        if (slashIndex >= 0)
            normalized = normalized[..slashIndex];

        int portSeparator = normalized.LastIndexOf(':');

        return portSeparator > -1 ? normalized[..portSeparator] : normalized;
    }
}
