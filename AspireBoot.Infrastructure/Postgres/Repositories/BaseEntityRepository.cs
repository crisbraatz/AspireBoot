...
public class BaseEntityRepository<T>(DatabaseContext databaseContext) : IBaseEntityRepository<T> where T : BaseEntity
{
    ...
    public async Task<IEnumerable<T>> SelectManyByAsync(
        ...
        track
            ? await DbSet.Where(filter ?? (x => true)).ToListAsync(cancellationToken).ConfigureAwait(false)
            : await DbSet
                .AsNoTracking().Where(filter ?? (x => true)).ToListAsync(cancellationToken).ConfigureAwait(false);

    public async Task<T?> SelectOneByAsync(
        Expression<Func<T, bool>>? filter = null, bool track = false, CancellationToken cancellationToken = default) =>
        track
            ? await DbSet.SingleOrDefaultAsync(filter ?? (x => true), cancellationToken).ConfigureAwait(false)
            : await DbSet
                .AsNoTracking().SingleOrDefaultAsync(filter ?? (x => true), cancellationToken).ConfigureAwait(false);
