...
public sealed class AuthController(IAuthService authService) : BaseController(authService)
{
    ...
    public async Task<IActionResult> RefreshTokenAsync(CancellationToken cancellationToken = default)
    {
        ...
        BaseResponseDto<RefreshTokenResponseDto> serviceResponse = await _authService
            .RefreshTokenAsync(new RefreshTokenRequestDto(refreshToken, GetClaimFromAuthorization()), cancellationToken)
        ...
        [FromBody] SignInUserRequest request, CancellationToken cancellationToken = default)
    {
        BaseResponseDto signInResponse = await _authService
            .SignInAsync(request.ConvertToDto(GetClaimFromAuthorization()), cancellationToken).ConfigureAwait(false);
        ...
        BaseResponseDto<RefreshTokenResponseDto> refreshTokenResponse = await _authService
            .RefreshTokenAsync(new RefreshTokenRequestDto(null, request.Email, false), cancellationToken)
            .ConfigureAwait(false);
        ...
        [FromBody] SignUpUserRequest request, CancellationToken cancellationToken = default)
    {
        BaseResponseDto signUpResponse = await _authService
            .SignUpAsync(request.ConvertToDto(GetClaimFromAuthorization()), cancellationToken).ConfigureAwait(false);
