...
2. Execute the command `npm install` in the `AspireBoot.Angular` directory
3. Create a local CA by executing the command in the `AspireBoot.AppHost` directory:
    ```bash
    ...
    ```
4. In the .NET IDE, run/debug the `AspireBoot.AppHost` project using the `https` profile
